<app>
    <div class="flex flex-col md:flex-row gap-6 justify-between md:items-center mb-2">
        ...
        <div class="flex gap-4">
            ...
            <flux:custom.button-create-permission :routeName="'persetujuan'" />
        </div>
    </div>
        ...
        <x-table>
            <x-table-heading>
                <x-table-heading-row>
                    ...
                    <x-table-heading-data>NAMA PERSETUJUAN</x-table-heading-data>
                    <x-table-heading-data>NAMA MODEL</x-table-heading-data>
            ...
            <x-table-body>
                @foreach($fetch as $key => $value)
                    <x-table-body-row :class="$loop->last ? 'border-none' : 'border-b'">
                        ...
                        <x-table-body-data>{{ $value->model_type }}</x-table-body-data>
                        <x-table-body-data :class="'text-right'">
                            <flux:custom.button-list-permission :id="$value->id" :routeName="'persetujuan'" />
                        </x-table-body-data>
                    </x-table-body-row>
                @endforeach
            </x-table-body>
