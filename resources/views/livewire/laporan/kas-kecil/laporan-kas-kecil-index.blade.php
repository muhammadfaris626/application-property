<app>
    <div class="flex flex-col md:flex-row gap-6 justify-between md:items-center mb-4">
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item divider="slash">Kas Kecil</flux:breadcrumbs.item>
        </flux:breadcrumbs>
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div class="md:col-span-2 flex flex-col gap-4">
                <div class="relative flex-1 rounded-lg bg-zinc-50 text-center px-4 pt-4">
                    <flux:custom.column-chart-basic :data="$columnChartKasKecil" :title="'Pemasukan dan Pengeluaran'" :label="'kas-kecil'" :useRupiah="true" />
                </div>
            </div>
            <div class="md:col-span-2 flex flex-col gap-4">
                <div class="relative flex-1 rounded-lg bg-zinc-50 text-center px-4 pt-4">
                    <flux:custom.bar-chart :label="'bar-kas-kecil'" :data="$barChartKasKecil" :category="$categoryYears" :title="'Pemasukan dan Pengeluaran (' . \Carbon\Carbon::now()->year - 2 . ' - ' . \Carbon\Carbon::now()->year . ')'" :useRupiah="true" />
                </div>
            </div>
                    ...
                    <x-table-body>
                        @foreach($fetchData as $key => $value)
                            <x-table-body-row :class="$loop->last ? 'border-none' : 'border-b'">
                                ...
                                <x-table-body-data>
                                    {{ 'Rp. ' . number_format($value->listKasKecils->sum('total'), 0, ',', '.') }}
                                </x-table-body-data>
                            </x-table-body-row>
                        @endforeach
                    </x-table-body>
