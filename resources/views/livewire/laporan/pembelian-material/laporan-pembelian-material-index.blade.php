<app>
    ...
    <div class="grid grid-cols-1 gap-4 mt-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <!-- Total Pembelian, Jumlah Material, dan Jumlah Faktur pada grid pertama (vertikal) -->
            <div class="md:col-span-1 flex flex-col gap-4">
                <div class="relative rounded-lg p-6 bg-zinc-50 dark:bg-zinc-700">
                    <flux:subheading>Total Pembelian</flux:subheading>
                    <flux:heading size="xl">Rp {{ number_format($totalPembelian, 0, ',', '.') }}</flux:heading>
                </div>

                <div class="relative rounded-lg p-6 bg-zinc-50 dark:bg-zinc-700">
                    <flux:subheading>Jumlah Material</flux:subheading>
                    <flux:heading size="xl">{{ $jumlahMaterial }}</flux:heading>
                </div>

                <div class="relative rounded-lg p-6 bg-zinc-50 dark:bg-zinc-700">
                    <flux:subheading>Jumlah Faktur</flux:subheading>
                    <flux:heading size="xl">{{ $jumlahFaktur }}</flux:heading>
                </div>
            </div>
            <!-- Grid kedua dan ketiga untuk chart -->
            <div class="md:col-span-2 flex flex-col gap-4">
                <div class="relative flex-1 rounded-lg bg-zinc-50 text-center px-4 pt-4">
                    <flux:custom.column-chart-basic :data="$columnChartPembelian" :title="'Total Pembelian'" :label="'chart-pembelian'" :useRupiah="true" />
                </div>
            </div>
