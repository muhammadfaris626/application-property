<app>
    <div class="flex flex-col md:flex-row gap-6 justify-between md:items-center mb-6">
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item :href="route('jenis-pengeluaran.index')" divider="slash">Jenis Pengeluaran</flux:breadcrumbs.item>
        ...
        <form wire:submit.prevent="update">
            <div class="border rounded-lg p-4 dark:border-white/10">
                <div class="grid xl:grid:cols-2 lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1 gap-4">
                    <div>
                        <flux:input wire:model="name" label="Jenis Pengeluaran" badge="Required" />
                    </div>
                </div>
            </div>
            <flux:custom.confirm-update :route="route('jenis-pengeluaran.index')" />
        </form>
    </div>
</app>
