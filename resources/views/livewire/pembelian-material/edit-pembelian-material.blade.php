<app>
    ...
    <div class="grid grid-cols-1">
        <form wire:submit.prevent="update">
            <flux:heading size="xl" class="px-4 pb-2">Ubah Pembelian Material</flux:heading>
                ...
                <div class="grid xl:grid-cols-2 lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1 gap-4">
                    ...
                    <div>
                        <flux:field>
                            ...
                            <flux:select wire:model="supplier_id" variant="listbox" placeholder="Pilih Supplier" :options="$fetchSupplier" :selectedData="$supplier_id" />
                    ...
                    <div>
                        <flux:field>
                            ...
                            <flux:select wire:model="employee_id" variant="listbox" placeholder="Pilih Penerima Barang" :options="$fetchKaryawan" :selectedData="$employee_id" />
                        ...
                        <div class="border rounded-lg dark:border-white/10 mx-4">
                            ...
                            <dl class="border-t divide-y divide-gray-100 dark:divide-white/10">
                                <div class="grid grid-cols-1 md:grid-cols-5 gap-4 m-4">
                                    <div class="md:col-span-2">
                                        <flux:field>
                                            ...
                                            <flux:select wire:model="allMaterials.{{ $key }}.material_id"
                                                ...
                                                :options="$fetchMaterial" :selectedData="$allMaterials[$key]['material_id']" />
                                    ...
                                    <div
                                    x-data="rupiahDisplay()"
                                    x-init="updateRaw(@js($allMaterials[$key]['total_price'] ?? 0))"
                                    wire:key="material-total-{{ $key }}"
                                >
                                    <flux:input.group label="Total Harga" badge="Otomatis">
                                        <flux:input.group.prefix>Rp</flux:input.group.prefix>
                                        <flux:input x-model="formatted" disabled />
                                    </flux:input.group>
                                </div>
                                </div>
                            </dl>
            ...
            <flux:custom.confirm-update :route="route('pembelian-material.index')" />
        </form>
    </div>
    <script>
