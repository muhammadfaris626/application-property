<app>
    <div class="flex flex-col md:flex-row gap-6 justify-between md:items-center mb-6">
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item :href="route('pemasukan-kas-kecil.index')" divider="slash">Kas Kecil</flux:breadcrumbs.item>
            <flux:breadcrumbs.item divider="slash">Tambah</flux:breadcrumbs.item>
        </flux:breadcrumbs>
    </div>
    <div class="grid grid-cols-1">
        <form wire:submit.prevent="store">
            <flux:heading size="xl" class="px-4 pb-2">Tambah Kas Kecil</flux:heading>
                ...
                <div class="grid xl:grid-cols-2 lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1 gap-4">
                    ...
                    <div>
                        <flux:field>
                            ...
                            <flux:select wire:model="employee_id" variant="listbox" placeholder="Pilih Penanggung Jawab" :options="$fetchPenanggungJawab" />
            ...
            <flux:heading size="xl" class="px-4 py-2">Tambah Pemasukan</flux:heading>
            <div class="border rounded-lg dark:border-white/10">
                <div class="grid grid-cols-1 gap-4 mt-4">
                    ...
                    @foreach($allList as $key => $value)
                        <div class="border rounded-lg dark:border-white/10 mx-4">
                            <div class="flex flex-row justify-between items-center px-5 py-2">
                                <h3 class="text-base/7 font-semibold text-gray-900 dark:text-gray-200">PEMASUKAN #{{ $key + 1 }}</h3>
                                <flux:button icon="trash" variant="danger" size="xs" wire:click.prevent="removePemasukan({{ $key }})"></flux:button>
                            </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-5 gap-4 m-4">
                                    <div class="md:col-span-2">
                                        <flux:field>
                                            ...
                                            <flux:select wire:model="allList.{{ $key }}.type_of_income_id"
                                                variant="listbox" placeholder="Pilih Jenis Pemasukan"
                                                :options="$fetchJenisPemasukan" />
                                            <flux:error name="allList.{{ $key }}.type_of_income_id" />
                                        </flux:field>
                                    </div>
                                    <div class="md:col-span-2">
                                        <flux:input
                    ...
                    <div class="flex justify-center mb-4">
                        <flux:button variant="primary" size="sm" icon="plus" wire:click.prevent="addPemasukan">
                            Tambah Pemasukan
                        </flux:button>
                    </div>
                </div>
            </div>
            <flux:custom.confirm-create :route="route('pemasukan-kas-kecil.index')" />
        </form>
    </div>
    <script>
